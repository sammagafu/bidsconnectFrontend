<template>
  <VerticalLayout>
      ...
      <Toolbar class="mb-6">
        <template #start>
          ...
          <Button
            ...
            :disabled="!selectedProcesses || !selectedProcesses.length"
          />
        </template>
        <template #end>
    ...
    <Dialog
      ...
      header="Procurement Process Details"
      :modal="true"
    ...
    >
      <div class="flex flex-col gap-6">
        <div>
          ...
          <InputText
            id="name"
            v-model.trim="procurementProcess.name"
            required="true"
            autofocus
            :invalid="submitted && !procurementProcess.name"
            fluid
            maxlength="100"
          />
          <small v-if="submitted && !procurementProcess.name" class="text-red-500"
        ...
        <div>
          ...
          <Select
            id="type"
            v-model="procurementProcess.type"
            ...
            fluid
          />
          <small v-if="submitted && !procurementProcess.type" class="text-red-500"
        ...
        <div>
          ...
          <Textarea
            id="description"
            v-model="procurementProcess.description"
            rows="3"
            cols="20"
            fluid
          />
    ...
    <Dialog
      v-model:visible="deleteProcessDialog"
    ...
    >
      <div class="flex items-center gap-4">
        ...
        <span v-if="procurementProcess"
          >Are you sure you want to delete <b>{{ procurementProcess.name }}</b
          >?</span
        >
      </div>
      <template #footer>
    ...
    <Dialog
      v-model:visible="deleteProcessesDialog"
      :style="{ width: '50vw', maxWidth: '500px', minWidth: '300px' }"
      header="Confirm"
      :modal="true"
      class="delete-dialog"
...
const fetchProcurementProcesses = async () => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Failed to fetch procurement processes',
      life: 3000,
    });
  } finally {
    loading.value = false;
...
const saveProcurementProcess = async () => {
  ...
  try {
    if (procurementProcess.value.slug) {
      await api.put(
        `tenders/procurement-processes/${procurementProcess.value.slug}/`,
        procurementProcess.value
      );
      toast.add({
        severity: 'success',
        summary: 'Successful',
        detail: 'Procurement Process Updated',
        life: 3000,
      });
    } else {
      ...
      toast.add({
        severity: 'success',
        summary: 'Successful',
        detail: 'Procurement Process Created',
        life: 3000,
      });
    }
    await fetchProcurementProcesses();
    ...
    if (error.response && error.response.status === 400) {
      ...
      toast.add({
        severity: 'error',
        summary: 'Error',
        detail: 'Failed to save procurement process',
        life: 3000,
      });
    }
  }
...
const deleteProcess = async () => {
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Successful',
      detail: 'Procurement Process Deleted',
      life: 3000,
    });
  } catch (error) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Failed to delete procurement process',
      life: 3000,
    });
  }
};
...
const deleteSelectedProcesses = async () => {
  try {
    await Promise.all(
      selectedProcesses.value.map((proc) =>
    ...
    toast.add({
      severity: 'success',
      summary: 'Successful',
      detail: 'Procurement Processes Deleted',
      life: 3000,
    });
  } catch (error) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Failed to delete procurement processes',
      life: 3000,
    });
  }
};
