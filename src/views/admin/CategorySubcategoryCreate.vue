...
const deleteSubcategory = () => {
  ...
  toast.add({
    severity: 'success',
    summary: 'Removed',
    detail: 'Subcategory marked for deletion',
    life: 3000,
  });
};
...
const saveCategory = async () => {
  ...
  if (category.value.subcategories.some((sub) => !sub.name.trim())) {
    toast.add({
      severity: 'warn',
      summary: 'Validation Error',
      detail: 'All subcategory names are required',
      life: 3000,
    });
    return;
  }
  ...
  try {
    ...
    if (category.value.id) {
      ...
      toast.add({
        severity: 'success',
        summary: 'Created',
        detail: `Category "${res.data.name}" created`,
        life: 3000,
      });
    }
    hideDialog();
...
const deleteSelectedCategories = async () => {
  try {
    await Promise.all(
      selectedCategories.value.map((cat) => api.delete(`tenders/categories/${cat.slug}/`))
    );
    toast.add({
      severity: 'success',
      summary: 'Deleted',
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Failed to delete selected',
      life: 3000,
    });
  }
};
