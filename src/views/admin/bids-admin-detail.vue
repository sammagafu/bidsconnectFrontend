      ...
      <b-card class="mb-4">
        <b-card-header class="d-flex justify-content-between align-items-center py-3">
          ...
          <div class="btn-group" role="group" aria-label="actions">
            ...
            <b-button
              ...
              @click="awardBid"
              v-if="bid && bid.status === 'under_evaluation' && !tender?.awarded_bid"
            >
            ...
            <b-button
              ...
              @click="rejectBid"
              v-if="bid && bid.status !== 'rejected' && bid.status !== 'accepted'"
            >
        ...
        <b-card-body v-if="bid && !loading">
          ...
          <div class="border p-3 rounded bg-body-tertiary mb-4">
            ...
            <p class="mb-1">
              <strong>Submission Date:</strong> {{ formatDate(bid.submission_date) }}
            </p>
            <p class="mb-1">
            ...
            <p class="mb-1">
              <strong>Proposed Completion Days:</strong> {{ bid.proposed_completion_days || 'N/A' }}
            </p>
          ...
          <b-alert v-else variant="warning" show> Tender data unavailable. </b-alert>
          ...
          <b-tabs content-class="mt-3" v-if="bid">
              ...
              >
                <template #cell(actions)="row">
                  <b-button
                    ...
                    @click="viewDocument(row.item.file)"
                  >
                    View Document
                  </b-button>
                </template>
            ...
            <b-tab title="Evaluations">
              ...
              />
              <b-alert
                v-if="!bid.bids_evaluations || bid.bids_evaluations.length === 0"
                variant="info"
                show
              >
                No evaluations yet.
            ...
            <b-tab title="Audit Logs">
              ...
              />
              <b-alert
                v-if="!bid.bids_audit_logs || bid.bids_audit_logs.length === 0"
                variant="info"
                show
              >
                No audit logs.
...
async function awardBid() {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Failed to award bid: ' + (parseApiError(err) || err.message),
    });
  }
}
...
async function rejectBid() {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Failed to reject bid: ' + (parseApiError(err) || err.message),
    });
  }
}
