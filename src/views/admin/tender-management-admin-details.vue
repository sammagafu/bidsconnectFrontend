      ...
      <b-card class="mb-4">
        <b-card-header class="d-flex justify-content-between align-items-center py-3">
          ...
          <div class="btn-group" role="group" aria-label="actions">
            ...
            <b-button
              ...
              v-if="tender && !tender.awarded_bid"
            >
        ...
        <b-card-body v-if="tender">
          ...
          <div class="border p-3 rounded bg-body-tertiary mb-4">
            ...
            <p class="mb-1">
              <strong>Subcategory:</strong> {{ tender.subcategory?.name || 'N/A' }}
            </p>
            <p class="mb-1">
              <strong>Procurement Process:</strong> {{ tender.procurement_process?.name || 'N/A' }}
            </p>
            <p class="mb-1"><strong>Agency:</strong> {{ tender.agency?.name || 'N/A' }}</p>
            <p class="mb-1"><strong>Status:</strong> {{ tender.status.toUpperCase() }}</p>
            <p class="mb-1">
              <strong>Publication Date:</strong> {{ formatDate(tender.publication_date) }}
            </p>
            <p class="mb-1">
              <strong>Submission Deadline:</strong> {{ formatDate(tender.submission_deadline) }}
            </p>
            ...
            <p class="mb-1">
              ...
              {{
                tender.awarded_bid ? getAwardedBidderName(tender.awarded_bid) : 'Not awarded yet'
              }}
            </p>
          ...
          <b-table :items="bids" :fields="bidFields" responsive striped hover small class="mb-0">
            <template #cell(bidder)="row">
              <router-link
                :to="{ name: 'admin.bids-detail', params: { id: row.item.id } }"
...
async function fetchTenderDetails() {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: parseApiError(err) || 'Could not load tender details.',
    });
    router.back();
  }
}
...
async function fetchBids() {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: parseApiError(err) || 'Could not load bids.',
    });
  }
}
...
async function awardTender() {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: parseApiError(err) || 'Failed to award tender.',
    });
  }
}
