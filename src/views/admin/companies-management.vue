    ...
    >
      <div class="d-flex flex-column gap-3">
        <div class="d-flex align-items-center gap-3">
          <i class="pi pi-exclamation-triangle text-danger" style="font-size: 2rem" />
          <span v-if="company"
            >Are you sure you want to decline approval for <b>{{ company.name }}</b
            >?</span
          >
        </div>
        <div>
          <label for="declineRemarks" class="d-block font-weight-bold mb-2"
          ...
          <Textarea
            ...
            required
          />
          <small v-if="submittedDecline && !declineRemarks" class="text-danger"
...
const fetchCompanies = async () => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: parseApiError(error) || 'Failed to fetch companies',
      life: 3000,
    });
  } finally {
    loading.value = false;
...
const saveCompany = async () => {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: parseApiError(error) || 'Failed to save company',
      life: 3000,
    });
  }
};
...
const deleteCompany = async () => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: parseApiError(error) || 'Failed to delete company',
      life: 3000,
    });
  }
};
...
const approveCompany = async (comp) => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: parseApiError(error) || 'Failed to approve company',
      life: 3000,
    });
  }
};
...
const declineCompany = async () => {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Successful',
      detail: 'Company Approval Declined',
      life: 3000,
    });
  } catch (error) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: parseApiError(error) || 'Failed to decline company approval',
      life: 3000,
    });
  }
};
...
const deleteSelectedCompanies = async () => {
  try {
    await Promise.all(selectedCompanies.value.map((comp) => companiesService.delete(comp.id)));
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: parseApiError(error) || 'Failed to delete companies',
      life: 3000,
    });
  }
};
