        ...
        <div class="d-flex justify-content-between align-items-center">
          ...
          <div>
            <b-button
              ...
              :disabled="!docs.length || exportLoading"
            >
            ...
            <b-button size="sm" variant="outline-secondary" class="ms-2" @click="fetchDocs()">
      ...
      <b-list-group flush>
        ...
        >
          <div>
            ...
            <small class="text-muted d-block"> Expires: {{ formatDate(doc.expires_at) }} </small>
          </div>
          <div>
            <a v-if="doc.document_file" :href="doc.document_file" target="_blank" class="me-3">
              Download
            </a>
            <b-button size="sm" variant="outline-primary" class="me-2" @click="openModal(doc)">
              <i class="bx bx-pencil"></i>
            </b-button>
            <b-button size="sm" variant="outline-danger" @click="removeDocument(doc.id)">
    ...
    >
      <b-form @submit.prevent="submit">
        ...
        <b-form-group label="Document Type*" label-for="dt">
          <b-form-select id="dt" v-model="form.document_type" :options="typeOptions" required />
        </b-form-group>

        <!-- Expiry Date -->
        <b-form-group label="Expiry Date*" label-for="ex">
          <b-form-input id="ex" v-model="form.expires_at" type="date" required />
        </b-form-group>

        <!-- File Picker -->
        <b-form-group :label="editing ? 'Replace File' : 'File (PDF <5MB)'">
          <input type="file" accept="application/pdf" @change="onFileChange" class="form-control" />
          <small v-if="form.file">{{ form.file.name }}</small>
        </b-form-group>

        <div class="d-flex justify-content-end mt-4">
          <b-button variant="secondary" class="me-2" @click="closeModal()"> Cancel </b-button>
...
async function fetchDocs() {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: parseApiError(e) || 'Could not load documents.',
    });
  }
}
...
async function exportDocuments() {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: parseApiError(e) || 'Export failed.',
    });
  } finally {
