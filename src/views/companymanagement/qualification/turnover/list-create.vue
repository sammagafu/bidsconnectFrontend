<template>
  <VerticalLayout>
    ...
    <!-- Create / Edit Modal -->
    <b-modal v-model="showDialog" :title="editing ? 'Edit Turnover' : 'New Turnover'" hide-footer>
        ...
        <b-row>
          ...
          <b-col md="12" class="mb-3">
            <b-form-group label="Proof Document" label-for="proof">
              <b-form-file id="proof" v-model="form.proof" accept=".pdf,.jpg,.png" />
            </b-form-group>
          </b-col>
        </b-row>
...
async function fetchItems(id) {
  if (!id) return;
  try {
    const { data } = await api.get(`accounts/companies/${id}/annual-turnovers/`);
...
async function onDelete(item) {
  ...
  try {
    await api.delete(`accounts/companies/${companyId.value}/annual-turnovers/${item.id}/`);
    toast.add({
      severity: 'success',
      summary: 'Deleted',
