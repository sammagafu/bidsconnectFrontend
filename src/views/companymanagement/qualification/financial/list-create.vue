        ...
        <div class="row">
          ...
          <div class="col-md-6 mb-3">
            <b-form-group label="Audit Report" label-for="audit_report">
              <b-form-file
                id="audit_report"
                v-model="form.audit_report"
                :state="Boolean(form.audit_report)"
                accept=".pdf,.doc,.docx,.xls,.xlsx"
              />
            </b-form-group>
          </div>
...
function fetchStatements(id) {
  if (!id) return;
  api
    .get(`accounts/companies/${id}/financial-statements/`)
    .then((r) => (statements.value = r.data))
    .catch(() =>
      toast.add({ severity: 'error', summary: 'Error', detail: 'Could not load statements.' })
    );
}
...
function saveStatement() {
  ...
  const call = isEditing.value
    ? api.patch(`${urlBase}/${form.year}/`, payload, {
  ...
  call
    ...
    .catch((err) => {
      toast.add({
        severity: 'error',
        summary: 'Error',
        detail: err.response?.data?.detail || 'Save failed.',
      });
    });
}
...
function onDelete(data) {
  ...
  api
    .delete(`accounts/companies/${companyId.value}/financial-statements/${data.year}/`)
    .then(() => {
      fetchStatements(companyId.value);
