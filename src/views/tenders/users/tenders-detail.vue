        ...
        <div v-else-if="tender">
          <table class="table table-bordered align-middle">
            <tbody>
              ...
              <tr>
                <td>Country</td>
                <td>{{ tender.tender_type_country }}</td>
              </tr>
              <tr>
                <td>Sector</td>
                <td>{{ tender.tender_type_sector }}</td>
              </tr>
              <tr>
                <td>Category / Subcategory</td>
                <td>
                  {{ tender.category?.name ?? 'N/A' }} / {{ tender.subcategory?.name ?? 'N/A' }}
                </td>
              </tr>
              <tr>
                <td>Procurement Process</td>
                <td>
                  {{ tender.procurement_process?.name ?? 'N/A' }} ({{
                    getProcurementTypeDisplay(tender.procurement_process?.type)
                  }})
                </td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td>{{ getSourceOfFundsDisplay(tender.source_of_funds) }}</td>
              </tr>
              <tr>
                <td>Re-advertised From</td>
                <td>{{ tender.re_advertised_from ? tender.re_advertised_from.title : 'N/A' }}</td>
              </tr>
              <tr>
                <td>Re-advertisement Count</td>
                <td>{{ tender.re_advertisement_count }}</td>
              </tr>
              <tr>
                <td>Allow Alternative Delivery</td>
                <td>{{ tender.allow_alternative_delivery ? 'Yes' : 'No' }}</td>
              </tr>
              ...
              <tr>
                <td>Published</td>
                <td>{{ formatDateTime(tender.publication_date) }}</td>
              </tr>
              <tr>
                <td>Submission Deadline</td>
                <td>{{ formatDateTime(tender.submission_deadline) }}</td>
              </tr>
              <tr>
                <td>Validity Period (Days)</td>
                <td>{{ tender.validity_period_days ?? 'N/A' }}</td>
              </tr>
              <tr>
                <td>Completion Period (Days)</td>
                <td>{{ tender.completion_period_days ?? 'N/A' }}</td>
              </tr>
              <tr>
                <td>Litigation History Start</td>
                <td>{{ formatDate(tender.litigation_history_start) }}</td>
              </tr>
              <tr>
                <td>Litigation History End</td>
                <td>{{ formatDate(tender.litigation_history_end) }}</td>
              </tr>
              ...
              <tr>
                <td>Tender Fees</td>
                <td>{{ tender.tender_fees }} {{ tender.currency }}</td>
              </tr>
              <tr>
                <td>Security Type</td>
                <td>{{ tender.tender_securing_type ?? 'N/A' }}</td>
              </tr>
              <tr v-if="tender.tender_security_percentage !== null">
                <td>Security %</td>
                <td>{{ tender.tender_security_percentage }}%</td>
              </tr>
              <tr v-if="tender.tender_security_amount !== null">
                <td>Security Amount</td>
                <td>{{ tender.tender_security_amount }} {{ tender.tender_security_currency }}</td>
              </tr>
              ...
              <tr>
                <td>Agency</td>
                <td>{{ tender.agency?.name ?? 'N/A' }}</td>
              </tr>
              <tr v-if="tender.agency?.phone_number">
                <td>Phone</td>
                <td>{{ tender.agency.phone_number }}</td>
              </tr>
              <tr v-if="tender.agency?.email">
                <td>Email</td>
                <td>{{ tender.agency.email }}</td>
              </tr>
                ...
                <td>
                  <ul v-if="tender.financial_requirements.length">
                    <li v-for="f in tender.financial_requirements" :key="f.id">
                      ...
                      <div v-if="f.jv_compliance">
                        JV Compliance: {{ getJvComplianceDisplay(f.jv_compliance) }}
                      </div>
                      <div v-if="f.jv_percentage !== null">
                        JV Percentage: {{ f.jv_percentage }}%
                      </div>
                ...
                <td>
                  <ul v-if="tender.turnover_requirements.length">
                    <li v-for="t in tender.turnover_requirements" :key="t.id">
                      ...
                      <div v-if="t.jv_compliance">
                        JV Compliance: {{ getJvComplianceDisplay(t.jv_compliance) }}
                      </div>
                      <div v-if="t.jv_percentage !== null">
                        JV Percentage: {{ t.jv_percentage }}%
                      </div>
                    </li>
                  </ul>
                  <span v-else>None</span>
                ...
                <td>
                  <ul v-if="tender.experience_requirements.length">
                    <li v-for="e in tender.experience_requirements" :key="e.id">
                      {{ getExperienceTypeDisplay(e.type) }} – {{ e.contract_count }} contracts ≥
                      {{ e.min_value }} {{ e.currency }}
                      ...
                      <div v-if="e.jv_compliance">
                        JV Compliance: {{ getJvComplianceDisplay(e.jv_compliance) }}
                      </div>
                      <div v-if="e.jv_percentage !== null">
                        JV Percentage: {{ e.jv_percentage }}%
                      </div>
                      <div v-if="e.jv_aggregation_note">
                        JV Aggregation Note: {{ e.jv_aggregation_note }}
                      </div>
                      <div v-if="e.reputation_notes">
                        Reputation Notes: {{ e.reputation_notes }}
                      </div>
                    </li>
                  </ul>
                  <span v-else>None</span>
                ...
                <td>
                  <ul v-if="tender.personnel_requirements.length">
                    <li v-for="p in tender.personnel_requirements" :key="p.id">
                      ...
                      <div v-if="p.min_education">
                        Education: {{ getEducationLevelDisplay(p.min_education) }}
                      </div>
                      <div v-if="p.professional_registration">
                        Professional Registration: {{ p.professional_registration }}
                      </div>
                      <div v-if="p.appointment_duration_years">
                        Appointment Duration: {{ p.appointment_duration_years }} yrs
                      </div>
                      <div v-if="p.nationality_required">
                        Nationality: {{ p.nationality_required }}
                      </div>
                      ...
                      <div v-if="p.specialized_education">
                        Specialized Education: {{ p.specialized_education }}
                      </div>
                      <div v-if="p.professional_certifications">
                        Certifications: {{ p.professional_certifications }}
                      </div>
                      <div v-if="p.jv_compliance">
                        JV Compliance: {{ getJvComplianceDisplay(p.jv_compliance) }}
                      </div>
              ...
              <tr>
                <td>Created By</td>
                <td>{{ tender.created_by?.email ?? 'N/A' }}</td>
              </tr>
              <tr>
                <td>Created At</td>
                <td>{{ formatDateTime(tender.created_at) }}</td>
              </tr>
              <tr>
                <td>Last Updated</td>
                <td>{{ formatDateTime(tender.updated_at) }}</td>
              </tr>
            </tbody>
          </table>
          ...
          <div class="d-flex gap-2 mb-4">
            <button class="btn btn-primary" @click="downloadPDF">Download Checklist PDF</button>
            <button
              class="btn btn-primary"
              @click="bidForTender(tender.slug)"
              ...
              <ul v-if="statusHistory.length" class="list-unstyled mb-0">
                <li v-for="(ev, i) in statusHistory" :key="i" class="d-flex gap-2 mb-2">
                  <span class="text-muted small">{{
                    formatDateTime(ev.created_at || ev.timestamp)
                  }}</span>
              ...
              <div class="mb-3" style="max-height: 200px; overflow-y: auto">
                <div v-for="(msg, i) in conversationMessages" :key="i" class="mb-2">
                  <small class="text-muted"
                    >{{ msg.sender?.email || 'User' }} · {{ formatDateTime(msg.created_at) }}</small
                  >
                  <p class="mb-0">{{ msg.content }}</p>
                </div>
                <p v-if="!conversationMessages.length && !chatLoading" class="text-muted mb-0">
