    ...
    <Dialog v-model:visible="showAddDialog" header="Add Tender Securing Declaration" modal :style="{ width: '50vw' }">
      <div class="p-fluid">
        <div class="p-field">
          ...
          <InputText id="procuring_entity" v-model="newItem.procuring_entity" class="w-full" />
        </div>
        <div class="p-field">
          <label for="tender_no">Tender No</label>
          <InputText id="tender_no" v-model="newItem.tender_no" class="w-full" />
        </div>
        <div class="p-field">
        ...
        <div class="p-field">
          ...
          <DatePicker id="date" v-model="newItem.date" showIcon class="w-full" />
        </div>
        <div class="p-field">
          <label for="signer_name">Signer Name</label>
          <InputText id="signer_name" v-model="newItem.signer_name" class="w-full" />
        </div>
        <div class="p-field">
          <label for="signer_capacity">Signer Capacity</label>
          <InputText id="signer_capacity" v-model="newItem.signer_capacity" class="w-full" />
        </div>
      </div>
...
async function fetchItems() {
  try {
    const res = await api.get('automation/tender-securing-declaration/');
...
async function submitNew() {
  try {
    ...
    const res = await api.post('automation/tender-securing-declaration/', newItem.value);
