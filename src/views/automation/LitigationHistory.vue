<!-- LitigationHistory.vue -->
<template>
  <VerticalLayout>
    <div class="card">
      <Toolbar>
        <template #end>
          <Button label="Add Litigation History" icon="pi pi-plus" @click="showAddDialog = true" />
        </template>
      </Toolbar>
...
async function fetchItems() {
  try {
    const res = await api.get('automation/litigation-history/');
...
async function submitNew() {
  try {
    ...
    const res = await api.post('automation/litigation-history/', newItem.value);
