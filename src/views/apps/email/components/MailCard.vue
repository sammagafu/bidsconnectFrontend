  ...
  <li :class="!mail.isRead && 'unread'">
    ...
    <div class="col-mail col-mail-2">
      <a @click="toggleReadOffcanvas" class="subject" role="button">
        ...
        <b-badge
          class="border border-secondary bg-body-tertiary text-body fw-semibold ms-1"
          v-for="(attach, idx) in mail.attachments"
          :key="idx"
        >
          <i class="bx bx-images text-primary me-2" v-if="mail.attachType === 'image'" />
          <i
            class="bx bxs-file-pdf text-danger text-primary me-2"
            v-if="mail.attachType === 'pdf'"
          />
          <i class="bx bxs-file-doc text-blue me-2" v-if="mail.attachType === 'doc'" />
          {{ attach }}
        </b-badge>
        <b-badge
          class="border border-secondary bg-body-tertiary text-body fw-semibold rounded-circle"
          v-if="mail.badge"
        >
          {{ mail.badge }}
        </b-badge>
