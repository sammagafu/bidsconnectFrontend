  ...
  <VerticalLayout>
    ...
    <Dialog v-model:visible="showAddQuoteDialog" header="Add Quote" modal>
      ...
      <div class="field">
        ...
        <Select
          v-model="newQuote.subcategory_id"
      ...
      <div class="field">
        <label>Items</label>
        <div
          v-for="(item, index) in newQuote.items"
          :key="index"
...
async function submitRequest() {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: err.response?.data?.detail || 'Failed to request invoice',
    });
  }
}
...
async function submitNewQuote() {
  try {
    ...
    if (newQuote.value.subcategory_id)
      formData.append('subcategory', newQuote.value.subcategory_id);
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: err.response?.data?.detail || 'Failed to add quote',
    });
  }
}
