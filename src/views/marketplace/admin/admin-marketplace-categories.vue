<!-- src/views/admin/components/CategoryManager.vue -->
<template>
  <VerticalLayout>
    <div class="card">
      <Toolbar class="mb-6">
        ...
        <Column header="Subcategory" style="min-width: 20rem">
          <template #body="slotProps">
            <div class="flex flex-wrap gap-2">
              <Chip
                v-for="(subcategory, index) in slotProps.data.subcategories"
                :key="index"
    ...
    <Dialog
      ...
      :style="{ width: '80vw', maxWidth: '800px', minWidth: '300px' }"
      ...
      <div class="flex flex-col gap-6">
        <div>
          ...
          <InputText
            ...
            fluid
          />
          <small v-if="submitted && !category.name" class="text-red-500"
        ...
        <div>
          ...
          <div v-if="isAddingSubcategory" class="flex flex-col gap-4 mb-4 p-4 border rounded">
            <div>
              <label for="new-subcategory-name" class="block font-bold mb-2"
                >New Subcategory Name</label
              >
              <InputText
            ...
            <Button label="Add" icon="pi pi-check" severity="success" @click="addSubcategory" />
          </div>
          <div
    ...
    >
      <div class="flex items-center gap-4">
        <i class="pi pi-exclamation-triangle !text-3xl" />
        <span v-if="subcategoryToDelete"
          >Are you sure you want to delete subcategory <b>{{ subcategoryToDelete.name }}</b
          >?</span
        >
      </div>
      <template #footer>
    ...
    <Dialog
      v-model:visible="editSubcategoryDialog"
      :style="{ width: '50vw', maxWidth: '500px', minWidth: '300px' }"
      ...
      <div class="flex flex-col gap-4">
        <div>
          ...
          <InputText
            ...
            v-model.trim="editedSubcategory.name"
            required="true"
            :invalid="submitted && !editedSubcategory.name"
            fluid
          />
          <small v-if="submitted && !editedSubcategory.name" class="text-red-500"
            >Subcategory name is required.</small
          >
        </div>
        <div>
          ...
          <Textarea
            id="edit-subcategory-desc"
            v-model="editedSubcategory.description"
            rows="3"
            cols="20"
            fluid
          />
...
const category = ref({ name: '', description: '', subcategories: [] });
...
const fetchSubcategories = async (categoryId) => {
  try {
    const response = await api.get('marketplaces/subcategories/', {
...
const deleteSubcategory = async () => {
  try {
    if (subcategoryToDelete.value.id) {
      await api.delete(`marketplaces/subcategories/${subcategoryToDelete.value.id}/`);
    }
...
const saveCategory = async () => {
  ...
  try {
    ...
    if (categoryId) {
      ...
      await Promise.all(
        category.value.subcategories.map(async (sub) => {
          if (sub.id) {
      ...
      if (category.value.subcategories.length > 0) {
        await Promise.all(
          category.value.subcategories.map((sub) =>
      ...
      toast.add({
        severity: 'success',
        summary: 'Successful',
        detail: `Category "${response.data.name}" Created`,
        life: 3000,
      });
    }
    await fetchCategories();
...
const deleteSelectedCategories = async () => {
  try {
    await Promise.all(
      selectedCategories.value.map((cat) => api.delete(`marketplaces/categories/${cat.id}/`))
    );
    await fetchCategories();
