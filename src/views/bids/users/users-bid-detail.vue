        ...
        <div v-else>
          ...
          <div v-if="tender" class="mb-4">
            ...
            <div class="border p-3 rounded bg-body-tertiary">
              ...
              <p class="mb-1">
                <strong>Agency:</strong> {{ tender.agency?.name || 'Unknown Agency' }}
              </p>
              <p class="mb-1">
                <strong>Reference Number:</strong> {{ tender.reference_number || 'N/A' }}
              </p>
              <p class="mb-1">
              ...
              <p class="mb-1">
                ...
                <span :class="['badge', getStatusClass(tender.status)]">{{
          ...
          <b-alert v-else variant="warning" show> Tender details not available. </b-alert>
            ...
            <Timeline :value="stages" layout="horizontal" align="center">
              <template #marker="slotProps">
                <span
                  ...
                  :style="getMarkerStyle(slotProps.item.value)"
                >
          ...
          <div v-if="bid" class="mb-4">
            ...
            <div class="border p-3 rounded bg-body-tertiary">
              <p class="mb-1">
                <strong>Total Price:</strong> {{ bid.total_price || 0 }} {{ bid.currency || '' }}
              </p>
              <p class="mb-1">
                <strong>Status:</strong>
              ...
              <p class="mb-1">
                <strong>Submission Date:</strong> {{ formatDateTime(bid.submission_date) }}
              </p>
              <p class="mb-1">
                <strong>Validity Complied:</strong> {{ bid.validity_complied ? 'Yes' : 'No' }}
              </p>
              <p class="mb-1">
                <strong>Completion Complied:</strong> {{ bid.completion_complied ? 'Yes' : 'No' }}
              </p>
              <p class="mb-1">
                <strong>Proposed Completion Days:</strong>
                {{ bid.proposed_completion_days || 'N/A' }}
              </p>
          ...
          <b-alert v-else variant="warning" show> Bid details not available. </b-alert>
          ...
          <b-tabs content-class="mt-3" v-if="bid">
            ...
            <b-tab title="Documents" active>
              ...
              <b-alert
                v-if="!bid.bids_documents || bid.bids_documents.length === 0"
                variant="info"
                show
              >
              ...
              <div v-if="bid.status === 'draft'" class="mt-4">
                ...
                <b-form @submit.prevent="uploadDocument">
                  <b-form-group label="Tender Document:" label-for="tender_document">
                    <b-form-select
                      v-model="newDocument.tender_document"
                      :options="
                        tender.required_documents.map((doc) => ({ value: doc.id, text: doc.name }))
                      "
                      required
                    ></b-form-select>
                  </b-form-group>
            ...
            <b-tab title="Financial Responses">
              ...
              <b-table
                striped
                hover
                :items="bid.bids_financial_responses || []"
                :fields="financialFields"
              ></b-table>
              <b-alert
                v-if="!bid.bids_financial_responses || bid.bids_financial_responses.length === 0"
                variant="info"
                show
              >
            ...
            <b-tab title="Turnover Responses">
              ...
              <b-table
                striped
                hover
                :items="bid.bids_turnover_responses || []"
                :fields="turnoverFields"
              ></b-table>
              <b-alert
                v-if="!bid.bids_turnover_responses || bid.bids_turnover_responses.length === 0"
                variant="info"
                show
              >
            ...
            <b-tab title="Experience Responses">
              ...
              <b-table
                striped
                hover
                :items="bid.bids_experience_responses || []"
                :fields="experienceFields"
              ></b-table>
              <b-alert
                v-if="!bid.bids_experience_responses || bid.bids_experience_responses.length === 0"
                variant="info"
                show
              >
            ...
            <b-tab title="Personnel Responses">
              ...
              <b-table
                striped
                hover
                :items="bid.bids_personnel_responses || []"
                :fields="personnelFields"
              ></b-table>
              <b-alert
                v-if="!bid.bids_personnel_responses || bid.bids_personnel_responses.length === 0"
                variant="info"
                show
              >
            ...
            <b-tab title="Office Responses">
              ...
              <b-table
                striped
                hover
                :items="bid.bids_office_responses || []"
                :fields="officeFields"
              ></b-table>
              <b-alert
                v-if="!bid.bids_office_responses || bid.bids_office_responses.length === 0"
                variant="info"
                show
              >
            ...
            <b-tab title="Source Responses">
              ...
              <b-table
                striped
                hover
                :items="bid.bids_source_responses || []"
                :fields="sourceFields"
              ></b-table>
              <b-alert
                v-if="!bid.bids_source_responses || bid.bids_source_responses.length === 0"
                variant="info"
                show
              >
            ...
            <b-tab title="Litigation Responses">
              ...
              <b-table
                striped
                hover
                :items="bid.bids_litigation_responses || []"
                :fields="litigationFields"
              ></b-table>
              <b-alert
                v-if="!bid.bids_litigation_responses || bid.bids_litigation_responses.length === 0"
                variant="info"
                show
              >
            ...
            <b-tab title="Schedule Responses">
              ...
              <b-table
                striped
                hover
                :items="bid.bids_schedule_responses || []"
                :fields="scheduleFields"
              ></b-table>
              <b-alert
                v-if="!bid.bids_schedule_responses || bid.bids_schedule_responses.length === 0"
                variant="info"
                show
              >
            ...
            <b-tab title="Technical Responses">
              ...
              <b-table
                striped
                hover
                :items="bid.bids_technical_responses || []"
                :fields="technicalFields"
              ></b-table>
              <b-alert
                v-if="!bid.bids_technical_responses || bid.bids_technical_responses.length === 0"
                variant="info"
                show
              >
              ...
              <div class="d-flex justify-content-between align-items-start flex-column flex-md-row">
                <div>
                  <h6 class="fw-bold text-dark mb-1">
                    {{ bidder.company?.name || 'Unknown Company' }}
                  </h6>
                  <p class="small text-muted mb-1">
                    Status:
                    <span :class="['badge', getStatusClass(bidder.status)]">{{
                      (bidder.status || '').toUpperCase()
                    }}</span>
                    | Total Price: {{ bidder.total_price || 0 }} {{ bidder.currency || '' }}
                  </p>
                  <p class="small text-muted mb-1">
...
const getStageIndex = (value) =>
  stages.value.findIndex((s) => s.value.toLowerCase() === value.toLowerCase());
