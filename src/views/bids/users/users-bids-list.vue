  ...
  <VerticalLayout>
    <b-container class="py-4">
      ...
      <b-overlay :show="loading" rounded="sm">
        ...
        <!-- Bids list -->
        <div v-else>
          <b-alert v-if="bids.length === 0" variant="info" show> No bids found </b-alert>

          <div v-for="bid in bids" :key="bid.id" class="border-bottom py-3">
            <div class="d-flex justify-content-between align-items-start flex-column flex-md-row">
              <div>
                <router-link
                  :to="{ name: 'company.bids-detail', params: { id: bid.id } }"
                ...
                <div class="d-flex gap-2">
                  ...
                  <span class="badge bg-danger">{{
                    daysLeft(bid.tender.submission_deadline)
                  }}</span>
                </div>
              </div>
              <div class="mt-3 mt-md-0">
                <b-button-group size="sm">
                  ...
                  <b-button
                    ...
                    :disabled="submittingBidId === bid.id || !bid.is_ready"
                    @click="submitBid(bid)"
                    :title="
                      bid.is_ready
                        ? ''
                        : `Missing requirements: ${bid.errors.join(', ')} - view details to fix`
                    "
                  >
                  ...
                  <b-button
                    variant="outline-primary"
                    :disabled="generatingReportId === bid.id"
                    @click="openingReport(bid)"
                  >
...
const fetchBids = async () => {
  ...
  for (const company of companies) {
    ...
    try {
      ...
      error.value =
        parseApiError(err) || 'Failed to fetch bids from some companies. Please try again.';
    }
  }
...
const submitBid = async (bid) => {
  ...
  if (
    confirm(
      `Are you sure you want to publish (submit) this bid for tender ${bid.tender.reference_number}? This action cannot be undone.`
    )
  ) {
