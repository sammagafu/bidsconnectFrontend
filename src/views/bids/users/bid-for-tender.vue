    ...
    <b-container>
      <b-row>
        <b-col cols="12">
          ...
          <b-card
            v-if="bidId && bidReadiness"
            class="mb-4"
            :class="bidReadiness.is_ready ? 'border-success' : 'border-warning'"
          >
            <template #header>
              <div class="d-flex align-items-center justify-content-between">
                <h5 class="mb-0">
                  <i
                    :class="[
                      'bx me-2',
                      bidReadiness.is_ready
                        ? 'bx-check-circle text-success'
                        : 'bx-error-circle text-warning',
                    ]"
                  ></i>
                  Bid Readiness
                </h5>
            ...
            <div v-else>
              ...
              <ul class="mb-0">
                <li v-for="(err, i) in bidReadiness.errors || []" :key="i" class="text-danger">
                  {{ err }}
                </li>
              </ul>
            </div>
          ...
          <b-overlay :show="loading" rounded="sm">
            ...
            <div class="accordion" role="tablist" v-if="tender">
              ...
              <b-card
                no-body
                class="mb-1"
                v-if="requiredDocuments.some((doc) => doc.name === 'Priced Schedule')"
              >
                <b-card-header
                  header-tag="header"
                ...
                >
                  ...
                  <i
                    :class="['pi', isPricedScheduleOpen ? 'pi-chevron-up' : 'pi-chevron-down']"
                  ></i>
                </b-card-header>
                <b-collapse
                  id="priced-schedule"
                  v-model="isPricedScheduleOpen"
                ...
                >
                  <b-card-body>
                    <b-form-group label="Bid Amount (TZS)">
                      <b-form-input
                        v-model.number="complianceData['Priced Schedule']"
                        type="number"
                    ...
                    <b-form-group label="VAT Amount (18%)">
                      <b-form-input
                        v-model.number="complianceData['VAT Amount']"
                        type="number"
                    ...
                    <b-button variant="primary" @click="complySection('priced-schedule')"
                      >Comply</b-button
                    >
                  </b-card-body>
                </b-collapse>
              ...
              <b-card no-body class="mb-1">
                ...
                >
                  ...
                  <i
                    :class="['pi', isPowerOfAttorneyOpen ? 'pi-chevron-up' : 'pi-chevron-down']"
                  ></i>
                </b-card-header>
                <b-collapse
                  id="power-of-attorney"
                  v-model="isPowerOfAttorneyOpen"
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    ...
                    <b-card v-if="selectedUser" class="mt-3">
                      ...
                      <ul class="list-unstyled">
                        ...
                        <li>
                          <strong>Experience:</strong> {{ selectedUser.years_of_experience }} years
                        </li>
                      </ul>
                    ...
                    <b-button variant="primary" @click="complySection('power-of-attorney')"
                      >Comply</b-button
                    >
                  </b-card-body>
                </b-collapse>
              ...
              <b-card
                ...
                v-if="
                  requiredDocuments.some(
                    (doc) => doc.name === 'Conformance to Technical Specifications'
                  )
                "
              >
                <b-card-header
                ...
                >
                  ...
                  <i
                    :class="[
                      'pi',
                      isTechnicalSpecificationsOpen ? 'pi-chevron-up' : 'pi-chevron-down',
                    ]"
                  ></i>
                </b-card-header>
                <b-collapse
                  id="technical-specifications"
                  v-model="isTechnicalSpecificationsOpen"
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <b-form-group label="Upload Compliance Statement">
                      <b-form-file
                        v-model="documents['Conformance to Technical Specifications']"
                        accept=".pdf"
                      />
                    </b-form-group>
                    <b-button variant="primary" @click="complySection('technical-specifications')"
                      >Comply</b-button
                    >
                  </b-card-body>
                </b-collapse>
              ...
              <b-card
                no-body
                class="mb-1"
                v-if="requiredDocuments.some((doc) => doc.name === 'Manufacturer Authorization')"
              >
                <b-card-header
                  header-tag="header"
                  class="p-2 bg-success pointer d-flex justify-content-between align-items-center"
                ...
                >
                  ...
                  <i
                    :class="[
                      'pi',
                      isManufacturerAuthorizationOpen ? 'pi-chevron-up' : 'pi-chevron-down',
                    ]"
                  ></i>
                </b-card-header>
                <b-collapse
                  id="manufacturer-authorization"
                  v-model="isManufacturerAuthorizationOpen"
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <b-form-group label="Upload Manufacturer Authorization Letter">
                      <b-form-file
                        v-model="documents['Manufacturer Authorization']"
                        accept=".pdf"
                      />
                    </b-form-group>
                    <b-button variant="primary" @click="complySection('manufacturer-authorization')"
                      >Comply</b-button
                    >
                  </b-card-body>
                </b-collapse>
              ...
              <b-card
                no-body
                class="mb-1"
                v-if="requiredDocuments.some((doc) => doc.name === 'Tender Security')"
              >
                <b-card-header
                  header-tag="header"
                  class="p-2 bg-success pointer d-flex justify-content-between align-items-center"
                ...
                >
                  ...
                  <i
                    :class="['pi', isTenderSecurityOpen ? 'pi-chevron-up' : 'pi-chevron-down']"
                  ></i>
                </b-card-header>
                <b-collapse
                  id="tender-security"
                  v-model="isTenderSecurityOpen"
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <b-form-group
                      :label="`Required Security: ${tender.tender_security_amount || tender.tender_security_percentage}% (${tender.tender_security_currency || 'TZS'})`"
                    >
                      <b-form-file v-model="documents['Tender Security']" accept=".pdf" />
                    </b-form-group>
                    <b-button variant="primary" @click="complySection('tender-security')"
                      >Comply</b-button
                    >
                  </b-card-body>
                </b-collapse>
              ...
              <b-card
                no-body
                class="mb-1"
                v-if="requiredDocuments.some((doc) => doc.name === 'litigation-history')"
              >
                <b-card-header
                  header-tag="header"
                  class="p-2 bg-success pointer d-flex justify-content-between align-items-center"
                ...
                <b-collapse
                  ...
                  v-model="isLitigationOpen"
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <p class="mb-3">
                      Tenderers are required to provide litigation records resulting from contracts
                      completed or ongoing under their execution (In case of Joint Venture,
                      compliance requirements are all Parties Combined - Must Meet requirements and
                      Each Member - Must Meet requirements).
                    </p>
                    ...
                    <b-button variant="primary" @click="addLitigation"> + Add Litigation </b-button>
                    <b-button variant="primary" @click="complySection('litigation-history')"
                      >Comply</b-button
                    >
                  </b-card-body>
                </b-collapse>
              ...
              <b-card
                no-body
                class="mb-1"
                v-if="requiredDocuments.some((doc) => doc.name === 'source-of-funds')"
              >
                <b-card-header
                  header-tag="header"
                  class="p-2 bg-success pointer d-flex justify-content-between align-items-center"
                ...
                >
                  <b-card-body>
                    ...
                    <b-button variant="primary" @click="complySection('source-of-funds')"
                      >Comply</b-button
                    >
                  </b-card-body>
                </b-collapse>
              ...
              <b-card
                no-body
                class="mb-1"
                v-if="requiredDocuments.some((doc) => doc.name === 'financial-statement')"
              >
                <b-card-header
                  header-tag="header"
                  class="p-2 bg-success pointer d-flex justify-content-between align-items-center"
                ...
                >
                  <b-card-body>
                    ...
                    <b-form-group label="Items per page" class="d-flex align-items-center">
                      ...
                      />
                      <span
                        >1 - {{ financialTableData.length }} of
                        {{ financialTableData.length }}</span
                      >
                    </b-form-group>
                    <b-button variant="primary" @click="addStatement">
                      <i class="pi pi-plus mr-2" /> Add Financial Statement
                    </b-button>
                    <b-button variant="primary" @click="complySection('financial-statements')"
                      >Comply</b-button
                    >
                  </b-card-body>
                </b-collapse>
              ...
              <b-card no-body class="mb-1" v-if="financialRequirements.length > 0">
                ...
                >
                  ...
                  <i
                    :class="[
                      'pi',
                      isFinancialRequirementsOpen ? 'pi-chevron-up' : 'pi-chevron-down',
                    ]"
                  ></i>
                </b-card-header>
                <b-collapse
                  id="financial-requirements"
                  v-model="isFinancialRequirementsOpen"
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <div v-for="req in financialRequirements" :key="req.id">
                      <b-card class="mt-3">
                        ...
                        <b-form-group label="Select Financial Statement">
                          <b-form-select
                            v-model="selectedStatementsForReq[req.id]"
                            :options="statementOptions"
                            value-field="value"
                            text-field="text"
                          />
                        ...
                        <b-form-group label="Actual Value">
                          <b-form-input
                            v-model="complianceData[req.name]"
                            type="number"
                            disabled
                            placeholder="Select statement to auto-fill"
                          />
                        </b-form-group>
                        <b-alert v-if="complianceErrors[req.name]" variant="danger" show>{{
                          complianceErrors[req.name]
                        }}</b-alert>
                      </b-card>
                    </div>
                    <b-button variant="primary" @click="complySection('financial-requirements')"
                      >Comply</b-button
                    >
                  </b-card-body>
                </b-collapse>
              ...
              <b-card no-body class="mb-1" v-if="turnoverRequirements.length > 0">
                ...
                >
                  ...
                  <i
                    :class="[
                      'pi',
                      isTurnoverRequirementsOpen ? 'pi-chevron-up' : 'pi-chevron-down',
                    ]"
                  ></i>
                </b-card-header>
                <b-collapse
                  id="turnover-requirements"
                  v-model="isTurnoverRequirementsOpen"
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <div v-for="req in turnoverRequirements" :key="req.id">
                      <b-card class="mt-3">
                        <h5>{{ req.label }}</h5>
                        <b-form-group
                          :label="'Minimum Required: ' + req.amount + ' ' + req.currency"
                        >
                        </b-form-group>
                        <b-form-group label="Select Turnover Years">
                          <b-form-checkbox-group
                            v-model="selectedTurnoversForReq[req.id]"
                        ...
                        <b-form-group label="Actual Amount">
                          <b-form-input
                            v-model="complianceData[req.label]"
                            type="number"
                            disabled
                            placeholder="Select turnovers to auto-fill"
                          />
                        </b-form-group>
                        <b-alert v-if="complianceErrors[req.label]" variant="danger" show>{{
                          complianceErrors[req.label]
                        }}</b-alert>
                      </b-card>
                    </div>
                    ...
                    <b-button variant="primary" @click="complySection('turnover-requirements')"
                      >Comply</b-button
                    >
                  </b-card-body>
                </b-collapse>
              ...
              <b-card no-body class="mb-1" v-if="experienceRequirements.length > 0">
                ...
                >
                  ...
                  <i
                    :class="[
                      'pi',
                      isExperienceRequirementsOpen ? 'pi-chevron-up' : 'pi-chevron-down',
                    ]"
                  ></i>
                </b-card-header>
                <b-collapse
                  id="experience-requirements"
                  v-model="isExperienceRequirementsOpen"
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <div v-for="req in experienceRequirements" :key="req.id">
                      <b-card class="mt-3">
                        ...
                        <b-form-group label="Select Company Experience">
                          <b-form-select
                            v-model="selectedExperiencesForReq[req.id]"
                        ...
                        <b-alert
                          v-if="complianceErrors[`Experience ${req.id}`]"
                          variant="danger"
                          show
                          >{{ complianceErrors[`Experience ${req.id}`] }}</b-alert
                        >
                      </b-card>
                    </div>
                    <b-button variant="primary" @click="complySection('experience-requirements')"
                      >Comply</b-button
                    >
                  </b-card-body>
                </b-collapse>
              ...
              <b-card no-body class="mb-1" v-if="personnelRequirements.length > 0">
                ...
                >
                  ...
                  <i
                    :class="[
                      'pi',
                      isPersonnelRequirementsOpen ? 'pi-chevron-up' : 'pi-chevron-down',
                    ]"
                  ></i>
                </b-card-header>
                <b-collapse
                  id="personnel-requirements"
                  v-model="isPersonnelRequirementsOpen"
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <div v-for="req in personnelRequirements" :key="req.id">
                      <b-card class="mt-3">
                        ...
                        <b-form-group
                          :label="
                            'Required: ' +
                            req.min_experience_yrs +
                            ' years, ' +
                            (req.professional_registration
                        ...
                        <b-form-group label="Select Personnel">
                          <b-form-select
                            v-model="assignedPersonnel[req.id]"
                        ...
                        <b-form-group label="Years of Experience">
                          <b-form-input
                            v-model.number="complianceData[`Personnel ${req.id} Experience`]"
                            type="number"
                            disabled
                            placeholder="Auto-filled from selected personnel"
                          />
                        </b-form-group>
                        <b-alert
                          v-if="complianceErrors[`Personnel ${req.id}`]"
                          variant="danger"
                          show
                          >{{ complianceErrors[`Personnel ${req.id}`] }}</b-alert
                        >
                      </b-card>
                    </div>
                    ...
                    <b-button variant="primary" @click="complySection('personnel-requirements')"
                      >Comply</b-button
                    >
                  </b-card-body>
                </b-collapse>
              ...
              <b-card no-body class="mb-1" v-if="tender.validity_period_days">
                ...
                >
                  ...
                  <i
                    :class="['pi', isTenderValidityOpen ? 'pi-chevron-up' : 'pi-chevron-down']"
                  ></i>
                </b-card-header>
                <b-collapse
                  id="tender-validity"
                  v-model="isTenderValidityOpen"
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <p class="mb-3">
                      Tenderers are required to confirm with the tender validity period specified by
                    ...
                    <p class="text-muted mb-3">
                      You have complied with the criteria. Please confirm that you complied and
                      continue with Submission
                    </p>
                    <b-form-checkbox v-model="validityComplied"> Complied </b-form-checkbox>
                    <b-button variant="primary" @click="complySection('tender-validity')"
                      >Comply</b-button
                    >
                  </b-card-body>
                </b-collapse>
              ...
              <b-card no-body class="mb-1" v-if="tender.completion_period_days">
                ...
                >
                  ...
                  <i
                    :class="['pi', isCompletionPeriodOpen ? 'pi-chevron-up' : 'pi-chevron-down']"
                  ></i>
                </b-card-header>
                <b-collapse
                  id="completion-period"
                  v-model="isCompletionPeriodOpen"
                  accordion="my-accordion"
                  role="tabpanel"
                >
                  <b-card-body>
                    <p class="mb-3">
                      Tenderers are required to comply with the completion period as proposed by the
                      procuring entity unless alternative completion period is allowed.
                    </p>
                    <h6>PE Requirement</h6>
                    <b-table
                      striped
                    ...
                    <b-form-checkbox v-model="completionComplied"> Complied </b-form-checkbox>
                    <b-form-group
                      label="Proposed Completion Days"
                      v-if="!completionComplied && tender.allow_alternative_delivery"
                    >
                      <b-form-input
                        v-model.number="proposedCompletionDays"
                        type="number"
                        placeholder="Enter proposed days"
                      />
                    </b-form-group>
                    <b-button variant="primary" @click="complySection('completion-period')"
                      >Comply</b-button
                    >
                  </b-card-body>
                </b-collapse>
            ...
            <b-button-group class="mt-4">
              <b-button variant="secondary" @click="saveDraft" :disabled="loading">
                Save Draft
              </b-button>
              <b-button variant="primary" @click="submitBid" :disabled="loading || !isFormComplete">
                Publish Bid
              </b-button>
            </b-button-group>
          </b-overlay>
...
const selectedUser = computed(
  () => personnel.value.find((p) => p.uuid === selectedUserId.value) || null
);

const litigationOptions = computed(() =>
  litigations.value.map((l) => ({
    value: l.id,
...
const selectedLitigations = computed(() =>
  litigations.value.filter((l) => selectedLitigationIds.value.includes(l.id))
);
...
const statementOptions = computed(() =>
  financialStatements.value.map((s) => ({
    value: s.id,
    text: `${s.year}`,
  }))
);
...
const selectedStatements = computed(() =>
  financialStatements.value.filter((s) => selectedStatementIds.value.includes(s.id))
);

const turnoverOptions = computed(() =>
  turnoverList.value.map((t) => ({
    value: t.id,
    text: `${t.year}`,
  }))
);
...
const turnoverTableData = computed(() =>
  turnoverList.value.map((t, index) => ({
    no: index + 1,
    year: t.year,
    currency: t.currency,
    amount: t.amount,
...
const financialTableData = computed(() =>
  financialStatements.value.map((s, index) => ({
    no: index + 1,
    year: s.year,
...
const completionPercentage = computed(() => {
  ...
  if (docNames.includes('Priced Schedule')) {
    total += 1;
    if (complianceData.value['Priced Schedule'] && complianceData.value['VAT Amount'])
      completed += 1;
  }
  ...
  turnoverRequirements.value.forEach((req) => {
    total += 1;
    if (
      selectedTurnoversForReq.value[req.id] &&
      selectedTurnoversForReq.value[req.id].length > 0 &&
      complianceData.value[req.label]
    ) {
  ...
  experienceRequirements.value.forEach((req) => {
    total += 1;
    if (selectedExperiencesForReq.value[req.id] && documents.value[`Experience ${req.id}`])
      completed += 1;
  });
  ...
  personnelRequirements.value.forEach((req) => {
    total += 1;
    if (
      assignedPersonnel.value[req.id] &&
      complianceData.value[`Personnel ${req.id} Experience`] >= req.min_experience_yrs
    )
      completed += 1;
  });
  ...
  if (tender.value.completion_period_days) {
    total += 1;
    if (
      completionComplied.value ||
      (proposedCompletionDays.value && tender.value.allow_alternative_delivery)
    )
      completed += 1;
  }
...
const isFormComplete = computed(
  () => completionPercentage.value === 100 && Object.keys(complianceErrors.value).length === 0
);
...
const validateCompliance = () => {
  ...
  financialRequirements.value.forEach((req) => {
    ...
    if (value && parseFloat(value) < parseFloat(req.minimum)) {
      complianceErrors.value[req.name] =
        `Value ${value} ${req.unit} is below required minimum ${req.minimum} ${req.unit}`;
    }
  });
  ...
  turnoverRequirements.value.forEach((req) => {
    ...
    if (value && parseFloat(value) < parseFloat(req.amount)) {
      complianceErrors.value[req.label] =
        `Amount ${value} ${req.currency} is below required minimum ${req.amount} ${req.currency}`;
    }
  });
  ...
  personnelRequirements.value.forEach((req) => {
    ...
    if (experience && experience < req.min_experience_yrs) {
      complianceErrors.value[`Personnel ${req.id}`] =
        `Experience (${experience} years) is below required minimum (${req.min_experience_yrs} years)`;
    }
  });
};
...
// Auto-calculate VAT
watch(
  () => complianceData.value['Priced Schedule'],
  (val) => {
    if (val) {
...
const addStatement = async () => {
  try {
    const response = await api.post(
      `accounts/companies/${company.value.id}/financial-statements/`,
      {
...
const viewFinancial = (item) => {
  toast.add({
    severity: 'info',
    summary: 'Viewing Financial Statement',
    detail: `Year: ${item.year}, Total Assets: ${item.total_assets}`,
  });
};
    ...
    if (drafts.length > 0) {
      ...
      (Array.isArray(persList) ? persList : []).forEach((pr) => {
        ...
        if (req) {
          ...
          complianceData.value[`Personnel ${req.id} Experience`] = personnel.value.find(
            (p) => p.uuid === pr.personnel
          )?.years_of_experience;
        }
      });
    }
...
const saveSimpleDocument = async (name, description = '') => {
  ...
  if (!file && documents.value[name] !== true)
    return toast.add({ severity: 'error', detail: 'Please upload document' });
...
const saveExperienceResponses = async () => {
  ...
  for (const req of experienceRequirements.value) {
    ...
    if (experienceId) {
      ...
      const existingId = existingMap.get(req.id);
      if (existingId) {
        await api.patch(`/bids/${bidId.value}/experience-responses/${existingId}/`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
      } else {
        await api.post(`/bids/${bidId.value}/experience-responses/`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
      }
    }
    ...
    switch (section) {
      case 'priced-schedule':
        if (!complianceData.value['Priced Schedule'] || !documents.value['Priced Schedule']) {
          return toast.add({
            severity: 'error',
            detail: 'Please enter amount and upload document',
          });
        }
        await saveBidScalars();
      ...
      case 'power-of-attorney':
        if (!selectedUserId.value || !documents.value['Power of Attorney']) {
          return toast.add({
            severity: 'error',
            detail: 'Please select representative and upload document',
          });
        }
...
const saveDraft = async () => {
  try {
    ...
    await saveBidScalars();
    await saveSimpleDocument('Priced Schedule');
    await saveSimpleDocument(
      'Power of Attorney',
      `Authorized representative: ${selectedUser.value?.first_name} ${selectedUser.value?.last_name}, ${selectedUser.value?.email}`
    );
    await saveSimpleDocument('Conformance to Technical Specifications');
    await saveSimpleDocument('Manufacturer Authorization');
    await saveSimpleDocument('Tender Security');
